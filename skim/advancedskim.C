...
void advancedskim(string inpath = "/nfs-6/userdata/mt2/V00-00-03", string outpath = "/tmp/gzevi", string tag = "ttall_msdecays") {
...
//  chain->SetBranchStatus("Particle.M2",1);
  ...
  //for(unsigned int evt = 0; evt < 100; evt++){  // For Testing

    reader.SetEntry(evt);
    //cout<<Electron_PT.GetSize()<<endl;

    // Event Selection
    int nlepPos = 0;
    int nlepNeg = 0;
    for (unsigned int i = 0; i < Electron_PT.GetSize() ; ++i) {
      //cout<<Electron_PT[i]<<endl;
      float pt = Electron_PT[i];
      int charge = Electron_Charge[i];
      if (pt < 20) continue;
      if (charge>0) nlepPos++;
      if (charge<0) nlepNeg++;
    }
